...
export const OrderResult: FC<{ displayType: UserRole | string, orders: Order[] }> = observer(({displayType,orders}) => {
    ...
    const makeTitleContent = ({user, partner}: Order) => {
        return `${displayType == UserRole.USER ? "Bạn" : user.fullName} Đã Đặt Dịch Vụ ${displayType == UserRole.USER ? "của " + partner.companyName : ""}`
    }
