...
const Root = styled.div((theme: Theme) => ({
    ...
    [`& .${classes.avatar}`]: { marginRight: "auto", marginLeft: "auto", marginBottom: "1rem", },
}));
...
export const Login: FC<{}> = observer(() => {
    ...
    const handleLogin = () => {
        if (!sSignIn.username || !sSignIn.password) {
            enqueueSnackbar(MESSAGE_TERMS.LOGIN_INVALID, { variant: "error" });
            return;
        }
        ...
        sSignIn
            .doLogin(redirect)
            .then((err) => {
                if (err) return enqueueSnackbar(MESSAGE_TERMS.get(err), { variant: "error" });
            })
